package com.caipiao.core.library.rule;
...
public class LhcPlayRule {
    ...
    static{
        for(int i = 1; i <= 49; i++) {
            if(DA(i)) DA.add(i);
            if(XIAO(i)) XIAO.add(i);
            if(DAN(i)) DAN.add(i);
            if(SHUANG(i)) SHUANG.add(i);
            if(WEIDA(i)) WEIDA.add(i);
            if(WEIXIAO(i)) WEIXIAO.add(i);
            if(HESHUDAN(i)) HESHUDAN.add(i);
            if(HESHUSHUANG(i)) HESHUSHUANG.add(i);
        }
    }
