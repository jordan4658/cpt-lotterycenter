...
/**
 * @Author: admin
 * @Description: 临时解决sg数据同步到caipiao库（一天的数据）， 防止orderserver 服务器这边长时间down 这种问题
 * @Version: 1.0.0
 * @Date; 2018/6/26 026 17:12
 */
@Service
public class SendSgOneDayHandler {
    private static final Logger logger = LoggerFactory.getLogger(SendSgOneDayHandler.class);

    @Autowired
    private SendSgService sendSgService;

    @XxlJob("SendSgOneDayHandler")
    public ReturnT<String> execute(String strings) throws Exception {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");

            sendSgService.sendSgOneDayData();
