 ...
 */
@Service
public class JsbjpksSgHandler {
    private static final Logger logger = LoggerFactory.getLogger(JsbjpksSgHandler.class);

    @Autowired
    private JsbjpksTaskService jsbjpksTaskService;

    @XxlJob("JsbjpksSgHandler")
    public ReturnT<String> executeJsbjpksSg(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.jsbjpksTaskService.addJsbjpksSg();
    ...
    @XxlJob("JsbjpksSgPrevHandler")
    public ReturnT<String> executeJsbjpksSgPrev(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.jsbjpksTaskService.addJsbjpksPrevSg();
            XxlJobLogger.log(name + "执行任务成功");
            return ReturnT.SUCCESS;
        } catch (Exception e) {
            logger.error(this.getClass().getName() + "执行任务失败,原因:" + e.getMessage(), e);
