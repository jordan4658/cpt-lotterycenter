 ...
 **/
@Service
public class OnelhcSgHandler {
    private static final Logger logger = LoggerFactory.getLogger(OnelhcSgHandler.class);

    @Autowired
    private OnelhcTaskService onelhcTaskService;

    @XxlJob("OnelhcSgHandler")
    public ReturnT<String> executeOnelhcSg(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.onelhcTaskService.addOnelhcSg();
    ...
    @XxlJob("OnelhcSgPrevHandler")
    public ReturnT<String> executeOnelhcSgPrev(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
