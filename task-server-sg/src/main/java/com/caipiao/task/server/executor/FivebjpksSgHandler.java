 ...
 */
@Service
public class FivebjpksSgHandler {
    private static final Logger logger = LoggerFactory.getLogger(FivebjpksSgHandler.class);

    @Autowired
    private FivebjpksTaskService fivebjpksTaskService;

    @XxlJob("FivebjpksSgHandler")
    public ReturnT<String> executeFivebjpksSg(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.fivebjpksTaskService.addFivebjpksSg();
    ...
    @XxlJob("FivebjpksSgPrevHandler")
    public ReturnT<String> executeFivebjpksSgPrev(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.fivebjpksTaskService.addFivebjpksPrevSg();
            XxlJobLogger.log(name + "执行任务成功");
            return ReturnT.SUCCESS;
        } catch (Exception e) {
            logger.error(this.getClass().getName() + "执行任务失败,原因:" + e.getMessage(), e);
