 ...
 **/
@Service
public class TensscSgHandler {
    private static final Logger logger = LoggerFactory.getLogger(TensscSgHandler.class);

    @Autowired
    private TensscTaskService tensscTaskService;

    @XxlJob("TensscSgHandler")
    public ReturnT<String> executeTensscSg(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
            this.tensscTaskService.addTensscSg();
    ...
    @XxlJob("TensscSgPrevHandler")
    public ReturnT<String> executeTensscSgPrev(String strings) {
        try {
            String name = this.getClass().getName();
            XxlJobLogger.log(name + "开始执行任务");
            // 从第三方接口获取赛果并添加到数据库
