 ...
 */
@Service
public class SslhcLotteryReadSgServiceImpl implements SslhcLotterySgServiceReadSg {
	private static final Logger logger = LoggerFactory.getLogger(SslhcLotteryReadSgServiceImpl.class);
	...
	public ResultInfo<Map<String, Object>> getSslhcNewestSgInfo() {
		...
		try {
			// 查询今日已售期数
			String openRedisKey = RedisKeys.AMLHC_OPEN_VALUE;
			...
			if (openCount == null) {
				...
				openCount = amlhcLotterySgMapperExt.openCountByExample(map);
				redisTemplate.opsForValue().set(openRedisKey, openCount);
			}
			// 获取开奖总期数
			Integer sumCount = CaipiaoSumCountEnum.AMLHC.getSumCount();
			// 计算当日剩余未开奖次数
			...
			AmlhcLotterySg nextAmlhcLotterySg = (AmlhcLotterySg) redisTemplate.opsForValue().get(nextRedisKey);

			if (nextAmlhcLotterySg == null) {
				nextAmlhcLotterySg = this.getNextAmlhcLotterySg();
			...
			if (ssllhcLotterySg == null) {
				// 查询最近一期开奖信息
				ssllhcLotterySg = this.getSslhcLotterySg();
			...
			if (nextAmlhcLotterySg != null) {
				...
				if (ssllhcLotterySg != null) {
					String number = ssllhcLotterySg.getNumber() == null ? Constants.DEFAULT_NULL : ssllhcLotterySg.getNumber();
					result.put(AppMianParamEnum.ISSUE.getParamEnName(),ssllhcLotterySg == null ? Constants.DEFAULT_NULL : ssllhcLotterySg.getIssue());
					result.put(AppMianParamEnum.NUMBER.getParamEnName(), number);
					result.put(AppMianParamEnum.SHENGXIAO.getParamEnName(), LhcUtils.getNumberZodiac(number, ssllhcLotterySg.getTime()));
				}
	...
	/* (non Javadoc) 
	 * @Title: getSslhcSgLong
	 * @Description: 获取时时六合彩长龙数据
	 * @return 
	 * @see com.caipiao.live.read.service.result.SslhcLotterySgService#getSslhcSgLong()
	 */ 
	@Override
	...
	*/ 
	public List<Map<String, Object>> addNextIssueInfo(List<Map<String, Object>> sslhcLongMapList, List<AmlhcLotterySg> sslhcLotterySgList){
		...
		if (!CollectionUtils.isEmpty(sslhcLongMapList)) {
			...
			if (nextAmlhcLotterySg == null) {
				nextAmlhcLotterySg = this.getNextAmlhcLotterySg();
				redisTemplate.opsForValue().set(nextRedisKey, nextAmlhcLotterySg, 10, TimeUnit.SECONDS);
			}
			if (nextAmlhcLotterySg == null) {
			...
			for (Map<String, Object> longMap : sslhcLongMapList) {
				longMap.put(AppMianParamEnum.NEXTISSUE.getParamEnName(), nextAmlhcLotterySg.getIssue());
				longMap.put(AppMianParamEnum.NEXTTIME.getParamEnName(),  DateUtils.getTimeMillis(nextAmlhcLotterySg.getIdealTime()) / 1000L);
	...
	*/ 
	public List<Map<String, Object>> getWallBigAndSmallLong(List<AmlhcLotterySg> amlhcLotterySgList){
		List<Map<String, Object>> fivelhcBigLongMapList = new ArrayList<Map<String, Object>>();
		// 特码两面单双
		Map<String, Object> twoWallDoubleAndSigleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCTMLMDOUBLEDRAGON.getTagType());
		// 特码两面大小
		Map<String, Object> twoWallBigAndSmallDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCTMLMBIGDRAGON.getTagType());
	...
	*/ 
	public List<Map<String, Object>> getTotalDoubleAndBigLong(List<AmlhcLotterySg> sslhcLotterySgList){
	...
	*/ 
	public List<Map<String, Object>> getZtsigleAndDoubleLong(List<AmlhcLotterySg> amlhcLotterySgList){
		List<Map<String, Object>> sslhcZtsigleAndDoubleMapList = new ArrayList<Map<String, Object>>();
		// 正1特单双
		Map<String, Object> firstSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZYTDOUBLEDRAGON.getTagType());
		// 正2特单双
		Map<String, Object> secondSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZETDOUBLEDRAGON.getTagType());
		// 正3特单双
		Map<String, Object> thirdSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZSTDOUBLEDRAGON.getTagType());
		// 正4特单双
		Map<String, Object> fourthSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZFTDOUBLEDRAGON.getTagType());
		// 正5特单双
		Map<String, Object> fivethSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZWTDOUBLEDRAGON.getTagType());
		// 正6特单双
		Map<String, Object> sixthSigleAndDoubleDragonMap = this.getPlayDragonMap(amlhcLotterySgList, CaipiaoPlayTypeEnum.AMLHCZLTDOUBLEDRAGON.getTagType());
	...
	*/ 
	public List<Map<String, Object>> getZtbigAndSmallLong(List<AmlhcLotterySg> amlhcLotterySgList){
		List<Map<String, Object>> sslhcZtbigAndSmallMapList = new ArrayList<Map<String, Object>>();
		// 正1特大小
		...
		if(firstBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(firstBigAndSmallDragonMap);
		}
		
		if(secondBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(secondBigAndSmallDragonMap);
		}
		
		if(thirdBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(thirdBigAndSmallDragonMap);
		}
		
		if(fourthBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(fourthBigAndSmallDragonMap);
		}
		
		if(fivethBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(fivethBigAndSmallDragonMap);
		}
		
		if(sixthBigAndSmallDragonMap.size() > Constants.DEFAULT_INTEGER) {
			sslhcZtbigAndSmallMapList.add(sixthBigAndSmallDragonMap);
		}
		return sslhcZtbigAndSmallMapList;
	}
	...
	*/ 
	public Map<String, Object> getPlayDragonMap(List<AmlhcLotterySg> sslhcLotterySgList, int type){
		...
		try {
			if (!CollectionUtils.isEmpty(sslhcLotterySgList)) {
				...
				for (int index = Constants.DEFAULT_INTEGER; index < sslhcLotterySgList.size() ; index++) {
					AmlhcLotterySg sslhcLotterySg = sslhcLotterySgList.get(index);
					String numberString = sslhcLotterySg.getNumber() == null ? Constants.DEFAULT_NULL : sslhcLotterySg.getNumber();
					// 按照玩法计算结果
	...
	*/ 
	public Map<String, Object> organizationSslhcDragonResultMap(int type, Set<String> dragonSet, Integer dragonSize){
		// 有龙情况下查询下期数据
		Map<String, Object> longDragonMap = new HashMap<String, Object>();
		try {
			// 玩法赔率
			Map<String, Object> oddsListMap = this.addAmlhcOddListInfo(type);
			// 把获取的赔率加入到返回MAP中
	...
	*/ 
	public Map<String, Object> addAmlhcOddListInfo(int type) {
        Map<String, Object> longDragonMap = new HashMap<String, Object>();
		// 赔率集合
	...
	*/ 
	public String calculateGoriResult(int type, String number) {
	...
	*/ 
	public AmlhcLotterySg getSslhcLotterySg() {
	...
	private AmlhcLotterySg getNextAmlhcLotterySg() {
		...
		AmlhcLotterySg nextAmlhcLotterySg = this.amlhcLotterySgMapper.selectOneByExample(nextExample);
		return nextAmlhcLotterySg;
	}
	...
	*/ 
	public List<AmlhcLotterySg> getSslhcAlgorithmData(){
